...
class Config:
    ...
    def __init__(self) -> None:
        ...
        # Trilium配置
        # ============================================
        self.trilium_base_url: str = os.getenv(
            "TRILIUM_BASE_URL", 
            "http://localhost:8080"
        )
        self.trilium_token: str = os.getenv("TRILIUM_TOKEN", "")
        self.trilium_data_dir: str = os.getenv(
            "TRILIUM_DATA_DIR", 
            "./data/trilium"
        )
        self.note_ids: list[str] = os.getenv(
            "TRILIUM_NOTE_IDS", 
            "root"
        ).split(",")
        ...
        # 语言模型配置
        # ============================================
        self.llm_model_path: str = os.getenv(
            "LLM_MODEL_PATH", 
            "./data/models/gpt4all/ggml-gpt4all-j-v1.3-groovy.bin"
        )
        ...
        # ============================================
        try:
            self.search_k: int = int(os.getenv(
                "SEARCH_K", 
                str(ConfigConstants.DEFAULT_SEARCH_K)
            ))
        except ValueError:
        ...
        # ============================================
        try:
            self.chunk_size: int = int(os.getenv(
                "CHUNK_SIZE", 
                str(ConfigConstants.DEFAULT_CHUNK_SIZE)
            ))
        except ValueError:
        ...
        try:
            self.chunk_overlap: int = int(os.getenv(
                "CHUNK_OVERLAP", 
                str(ConfigConstants.DEFAULT_CHUNK_OVERLAP)
            ))
        except ValueError:
        ...
        # 镜像源配置
        # ============================================
        self.hf_endpoint: str = os.getenv(
            "HF_ENDPOINT", 
            "https://hf-mirror.com"
        )
    ...
    def validate(self) -> None:
        ...
        if self.search_k > ConfigConstants.MAX_SEARCH_RESULTS:
            logger.warning(
                f"SEARCH_K ({self.search_k}) 超过最大值 "
                f"{ConfigConstants.MAX_SEARCH_RESULTS}，将使用最大值"
            )
        ...
        if self.llm_model_type.lower() not in valid_llm_types:
            errors.append(
                f"LLM_MODEL_TYPE 必须是 {valid_llm_types} 之一，当前值: {self.llm_model_type}"
            )
