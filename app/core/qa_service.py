...
答案："""
    ...
    async def ask_question_async(self, question: str, session_id: str = "default") -> dict[str, Any]:
        ...
        try:
            ...
            docs = await self.knowledge_base.semantic_search_async(
                question, 
    ...
    def ask_question(self, question: str, session_id: str = "default") -> dict[str, Any]:
        ...
        try:
            # 使用混合搜索策略提高检索准确性
            # 1. 基础相似度搜索
            docs_similarity = self.knowledge_base.vector_store.similarity_search(
                question, 
                k=self.config.search_k
            )
            ...
            docs_mmr = self.knowledge_base.vector_store.max_marginal_relevance_search(
                question, 
                k=self.config.search_k, 
                fetch_k=self.config.search_k * ConfigConstants.MMR_FETCH_K_MULTIPLIER,
                lambda_mult=ConfigConstants.MMR_LAMBDA_MULT  # 偏向相关性而非多样性
            )
    ...
    def _clean_answer(self, answer: str) -> str:
        ...
        indicators = ["答案：", "你的回答：", "回答：", "答案:", "回答:", "有帮助的回答:"]
