    ...
    def _connect(self) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.info("Successfully connected to Trilium.")
            except Exception:
                logger.warning("Warning: Could not verify Trilium connection (get_app_info failed).")
            ...
            for root_note_id in note_ids_to_process:
                ...
                while queue:
                    ...
                    if total_scanned > 0 and total_scanned % 50 == 0:
                        logger.info(f"Progress: Scanned {total_scanned} notes (Collected: {len(documents)}, Skipped: {skipped_count}, Queue: {len(queue)})...")
                    ...
                    try:
                        ...
                        for attempt in range(max_retries):
                            try:
                                ...
                                if "WinError 10048" in error_str or "Connection refused" in error_str:
                                    ...
                                    logger.warning(f"Socket exhaustion detected, waiting 5s... (Attempt {attempt+1}/{max_retries})")
                                    time.sleep(5)
                                elif attempt < max_retries - 1:
                                    time.sleep(retry_delay * (attempt + 1))
                                else:
                                    logger.error(f"Failed to get note {current_note_id} after {max_retries} attempts: {e}")
                        ...
                        if note_type not in ConfigConstants.VALID_NOTE_TYPES:
                            ...
                            skipped_count += 1
                        elif mime in binary_types or any(mime.startswith(prefix) for prefix in ConfigConstants.BINARY_MIME_PREFIXES):
                        ...
                        if should_process_content:
                            ...
                            try:
                                ...
                                timeout = ConfigConstants.TRILIUM_API_TIMEOUT
                                for attempt in range(max_retries):
                                    try:
                                        content_response = self.client.get_note_content(current_note_id, timeout=timeout)
                                        break
                                    except Exception as e:
                                        error_str = str(e)
                                        if "WinError 10048" in error_str:
                                            logger.warning(f"Socket exhaustion detected fetching content, waiting 5s... (Attempt {attempt+1}/{max_retries})")
                                            time.sleep(3)
                                        elif attempt < max_retries - 1:
                                            time.sleep(retry_delay * (attempt + 1))
                                        else:
                                            logger.error(f"Failed to fetch content for {title} after {max_retries} attempts: {e}")
                            ...
                            if content and len(content.strip()) > ConfigConstants.MIN_CONTENT_LENGTH:
                                ...
                                logger.debug(f"跳过内容过短的笔记: {title} (长度: {len(content.strip()) if content else 0})")
                                skipped_count += 1
